...
public class Block {
	...
	/**
	 * Flags whether or not this block is of a type that needs random ticking. Ref-counted by ExtendedBlockStorage in order to broadly cull a chunk from the random chunk handler list for efficiency's sake.
	 */
	 ...
	 * 
	 * @deprecated call via {@link IBlockState#useNeighborBrightness()} whenever possible. Implementing/overriding is fine.
	 */
	...
	protected static boolean isExceptionBlockForAttaching(Block attachBlock) {
		return attachBlock instanceof BlockShulkerBox || attachBlock instanceof BlockLeaves || attachBlock instanceof BlockTrapDoor || attachBlock == Blocks.BEACON || attachBlock == Blocks.CAULDRON || attachBlock == Blocks.GLASS
				|| attachBlock == Blocks.GLOWSTONE || attachBlock == Blocks.ICE || attachBlock == Blocks.SEA_LANTERN || attachBlock == Blocks.STAINED_GLASS;
	}
	...
	/**
	 * @deprecated call via {@link IBlockState#getBlockHardness(World,BlockPos)} whenever possible. Implementing/overriding is fine.
	 */
	...
	/**
	 * Sets whether this block type will receive random handler ticks
	 */
	...
	/**
	 * Returns whether or not this block is of a type that needs random ticking. Called for ref-counting purposes by ExtendedBlockStorage in order to broadly cull a chunk from the random chunk handler list for efficiency's sake.
	 */
	 ...
	 * 
	 * @deprecated call via {@link IBlockState#getSelectedBoundingBox(World,BlockPos)} whenever possible. Implementing/overriding is fine.
	 */
	...
	/**
	 * @deprecated call via {@link IBlockState#addCollisionBoxToList(World,BlockPos,AxisAlignedBB,List,Entity,boolean)} whenever possible. Implementing/overriding is fine.
	 */
	 ...
	 * 
	 * @deprecated call via {@link IBlockState#collisionRayTrace(World,BlockPos,Vec3d,Vec3d)} whenever possible. Implementing/overriding is fine.
	 */
	...
	/**
	 * Called on server when World#addBlockEvent is called. If server returns true, then also called on the client. On the Server, this may perform additional changes to the world, like pistons replacing the block with an extended base. On the client, the
	 * handler may involve replacing tile entities or effects such as sounds or particles
	 * 
	...
	/**
	 * @deprecated call via {@link IBlockState#getAmbientOcclusionLightValue()} whenever possible. Implementing/overriding is fine.
	 */
	...
	/**
	 * Called when an Entity lands on this Block. This method *must* handler motionY because the entity will not do that on its own
	 */
	...
	/**
	 * @deprecated call via {@link IBlockState#getOffset(IBlockAccess,BlockPos)} whenever possible. Implementing/overriding is fine.
	 */
	public Vec3d getOffset(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		if (block$enumoffsettype == EnumOffsetType.NONE) {
			...
			return new Vec3d(((double) ((float) (i >> 16 & 15L) / 15.0F) - 0.5D) * 0.5D, block$enumoffsettype == EnumOffsetType.XYZ ? ((double) ((float) (i >> 20 & 15L) / 15.0F) - 1.0D) * 0.2D : 0.0D,
