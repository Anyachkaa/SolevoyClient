...
public @Data class Minecraft implements IThreadListener, ISnooperInfo {
	...
	/**
	 * A 10MiB preallocation to ensure the heap is reasonably sized. {@linkplain #freeMemory() Freed} when the game runs out of memory.
	 *
	 * @see #freeMemory()
	 */
	...
	/**
	 * Time passed since the last handler in ticks. Used instead of this.timer.renderPartialTicks when paused in singleplayer.
	 */
	...
	private MCFontRenderer main;
	private MCFontRenderer main_2;
	private MCFontRenderer notification;
	private MCFontRenderer module;

	/**
	 * This is set to fpsCounter every debug screen handler, and is shown on the debug screen. It's also sent as part of the usage snooping.
	 */
	...
	/** Approximate time (in ms) of last handler to debug string */
	...
	private void init() throws Exception {
		...
		this.createDisplay();
		SolevoyClient.setDisplay("SolevoyClient by itskekoff#5954");
		...
		SolevoyClient.getInstance().load();
		...
		this.main = new MCFontRenderer(FontUtil.getFontFromTTF(new ResourceLocation("solevoyclient/font/main.ttf"), 16.0F, 0), true, true);
		this.main_2 = new MCFontRenderer(FontUtil.getFontFromTTF(new ResourceLocation("solevoyclient/font/hud-logo.ttf"), 21.5F, 0), true, true);
		this.module = new MCFontRenderer(FontUtil.getFontFromTTF(new ResourceLocation("solevoyclient/font/module.ttf"), 17.5F, 0), true, true);
		this.text = new MCFontRenderer(FontUtil.getFontFromTTF(new ResourceLocation("solevoyclient/font/comfortaa.ttf"), 17.5F, 0), true, true);
		...
		SolevoyClient.getInstance().getConfigManager().loadConfig("latest");
	}
	...
	public void displayGuiScreenFixed(@Nullable GuiScreen guiScreenIn) {
	 ...
	 *
	 * <p>
	 * <strong>WARNING</strong>: This method is not thread-safe. Opening GUIs from a thread other than the main thread may cause many different issues, including the GUI being rendered before it has initialized (leading to unusual crashes). If on a thread
	 * other than the main thread, use {@link #addScheduledTask}:
	 *
	 * <pre>
	...
	private void runGameLoop() throws IOException {
		...
		while (getSystemTime() >= this.debugUpdateTime + 1000L) {
			...
			this.debug = String.format("%d fps (%d chunk handler%s) T: %s%s%s%s%s", debugFPS, RenderChunk.renderChunksUpdated, RenderChunk.renderChunksUpdated == 1 ? "" : "s",
					(float) this.gameSettings.limitFramerate == GameSettings.Options.FRAMERATE_LIMIT.getValueMax() ? "inf" : this.gameSettings.limitFramerate, this.gameSettings.enableVsync ? " vsync" : "", this.gameSettings.fancyGraphics ? "" : " fast",
	...
	public void runTick() throws IOException {
		EventGameTick eventWorldTick = new EventGameTick(StateType.PRE);
		EventManager.call(eventWorldTick);
		...
		if (this.currentScreen == null || this.currentScreen.isAllowUserInput()) {
		...
		EventGameTick eventWorldTick1 = new EventGameTick(StateType.POST);
		EventManager.call(eventWorldTick1);
	}
		...
		while (Keyboard.next()) {
			...
			if (flag) {
				...
				if (this.currentScreen == null) {
					...
					EventKeyType eventKeyType = new EventKeyType(i);
					EventManager.call(eventKeyType);
				}
	...
	private void debugFeedbackTranslated(String untranslatedTemplate, Object... objs) {
		this.ingameGUI.getChatGUI().printChatMessage((new TextComponentString("")).appendSibling((new TextComponentTranslation("debug.prefix", new Object[0])).setStyle((new Style()).setColor(TextFormatting.YELLOW).setBold(Boolean.valueOf(true))))
				.appendText(" ").appendSibling(new TextComponentTranslation(untranslatedTemplate, objs)));
	}
