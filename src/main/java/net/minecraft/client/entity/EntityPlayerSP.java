...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void onUpdate() {
		if (this.world.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
			EventUpdate eventUpdate = new EventUpdate(this.posX, this.posY, this.posZ, this.motionX, this.motionY, this.motionZ, this.rotationYaw, this.rotationPitch);
	...
	private void onUpdateWalkingPlayer() {
		SolevoyClient.getInstance().setPosition(new Position(this.posX, this.posY, this.posZ, this.rotationYaw, this.rotationPitch));
	...
	public void sendChatMessage(String message) {
		EventSendMessage eventSendMessage = new EventSendMessage(message, this);
		EventManager.call(eventSendMessage);
		if (eventSendMessage.isCancelled()) return;
		this.connection.sendPacket(new CPacketChatMessage(message));
		GuiNewChat.printMessage("<" + mc.player.getName() + "> " + message);
	}
