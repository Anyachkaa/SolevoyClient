...
public class PlayerControllerMP implements IPlayerControllerMP {
    ...
    public EnumActionResult processRightClick(EntityPlayer player, World worldIn, EnumHand hand) {
        if (this.currentGameType == GameType.SPECTATOR) {
            ...
            try {
                SolevoyClient.getInstance().getMacroManager().getCurrentMacroRecord().getPackets().add(new ClientPlayerTryUseItemPacket());
                SolevoyClient.getInstance().getCurrentMotherRecord().getRecordPackets().add(new ClientPlayerTryUseItemPacket());
    ...
    /**
     * Used in PlayerControllerMP to handler the server with an ItemStack in a slot.
     */
