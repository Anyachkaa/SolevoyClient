		...
		if (!this.pathMap.isEmpty()) {
			...
			for (Integer integer : this.pathMap.keySet()) {
				...
				if (this.addDistanceToPlayer(pathpoint) <= 40.0F) {
					RenderGlobal.renderFilledBox((new AxisAlignedBB((double) ((float) pathpoint.x + 0.25F), (double) ((float) pathpoint.y + 0.25F), (double) pathpoint.z + 0.25D, (double) ((float) pathpoint.x + 0.75F),
							(double) ((float) pathpoint.y + 0.75F), (double) ((float) pathpoint.z + 0.75F))).offset(-this.xo, -this.yo, -this.zo), 0.0F, 1.0F, 0.0F, 0.5F);
					...
					for (int j = 0; j < path.getCurrentPathLength(); ++j) {
						...
						if (this.addDistanceToPlayer(pathpoint1) <= 40.0F) {
							...
							RenderGlobal.renderFilledBox((new AxisAlignedBB((double) ((float) pathpoint1.x + 0.5F - f), (double) ((float) pathpoint1.y + 0.01F * (float) j), (double) ((float) pathpoint1.z + 0.5F - f),
									(double) ((float) pathpoint1.x + 0.5F + f), (double) ((float) pathpoint1.y + 0.25F + 0.01F * (float) j), (double) ((float) pathpoint1.z + 0.5F + f))).offset(-this.xo, -this.yo, -this.zo), f1, 0.0F, f2, 0.5F);
						}
					}
				}
			}
