...
public class CommandSenderWrapper implements ICommandSender {
	...
	public CommandSenderWrapper withSendCommandFeedback(boolean sendCommandFeedbackIn) {
		return this.sendCommandFeedback == null || this.sendCommandFeedback.booleanValue() && !sendCommandFeedbackIn ? new CommandSenderWrapper(this.delegate, this.positionVector, this.position, this.permissionLevel, this.entity, sendCommandFeedbackIn)
				: this;
	}
	 ...
	 * <dd>For some entities, this will attempt to translate a nonexistent translation string; see <a href="https://bugs.mojang.com/browse/MC-68446">MC-68446</a>. For {@linkplain net.minecraft.entity.player.EntityPlayer#getName() players} this returns the
	 * player's name. For {@linkplain net.minecraft.entity.passive.EntityOcelot ocelots} this may return the translation of <code>entity.Cat.name</code> if it is tamed. For {@linkplain net.minecraft.entity.item.EntityItem#getName() item entities}, this will
	 * attempt to return the name of the item in that item entity. In all cases other than players, the custom name will overrule this.</dd>
	 ...
	 * <dd>For most entities, this returns the result of {@link #getName()}, with {@linkplain net.minecraft.scoreboard.ScorePlayerTeam#formatPlayerName scoreboard formatting} and a {@linkplain Entity#getHoverEvent special hover event}.</dd>
