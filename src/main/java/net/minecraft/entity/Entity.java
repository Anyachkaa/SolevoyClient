...
public abstract class Entity implements ICommandSender {
	...
	/**
	 * Attempts to write this Entity to the given NBTTagCompound. Returns false if the entity is dead or its string representation is null. In this event, the given NBTTagCompound is not modified.
	 * 
	...
	/**
	 * Either write this entity to the NBT tag given and return true, or return false without doing anything. If this returns false the entity is not saved on disk. Riding entities return false here as they are saved with their mount.
	 */
	...
	/**
	 * Applies this entity's orientation (pitch/yaw) to another entity. Used to handler passenger orientation.
	 */
	...
	/**
	 * Only used by renderer in EntityLivingBase subclasses. Determines if an entity is visible or not to a specific player, if the entity is normally invisible. For EntityLivingBase subclasses, returning false when invisible will render the entity
	 * semi-transparent.
	 */
	...
	/**
	 * Returns true if the flag is active for the entity. Known flags: 0: burning; 1: sneaking; 2: unused; 3: sprinting; 4: unused; 5: invisible; 6: glowing; 7: elytra flying
	 */
	...
	/**
	 * Gets the name of this thing. This method has slightly different behavior depending on the interface (for <a href="https://github.com/ModCoderPack/MCPBot-Issues/issues/14">technical reasons</a> the same method is used for both IWorldNameable and
	 * ICommandSender):
	 * 
	 * <dl>
	 * <dd>However, note that <strong>the translation string may be invalid</strong>, as is the case for {@link net.minecraft.tileentity.TileEntityBanner TileEntityBanner} (always returns nonexistent translation code <code>banner</code> without a custom name),
	 * {@link net.minecraft.block.BlockAnvil.Anvil BlockAnvil$Anvil} (always returns <code>anvil</code>), {@link net.minecraft.block.BlockWorkbench.InterfaceCraftingTable BlockWorkbench$InterfaceCraftingTable} (always returns <code>crafting_table</code>),
	 * {@link net.minecraft.inventory.InventoryCraftResult InventoryCraftResult} (always returns <code>Result</code>) and the {@link net.minecraft.entity.item.EntityMinecart EntityMinecart} family (uses the entity definition). This is not an exaustive
	 * list.</dd>
	 * <dd>In general, this method should be safe to use on tile entities that implement IInventory.</dd>
	 * <dt>{@link ICommandSender#getName() ICommandSender.getName()} and {@link Entity#getName() Entity.getName()}</dt>
	 * <dd>Returns a valid, displayable name (which may be localized). For most entities, this is the translated version of its translation string (obtained via {@link EntityList#getEntityString EntityList.getEntityString}).</dd>
	 * <dd>If this entity has a custom name set, this will return that name.</dd>
	 * <dd>For some entities, this will attempt to translate a nonexistent translation string; see <a href="https://bugs.mojang.com/browse/MC-68446">MC-68446</a>. For {@linkplain EntityPlayer#getName() players} this returns the player's name. For
	 * {@linkplain net.minecraft.entity.passive.EntityOcelot ocelots} this may return the translation of <code>entity.Cat.name</code> if it is tamed. For {@linkplain EntityItem#getName() item entities}, this will attempt to return the name of the item in that
	 * item entity. In all cases other than players, the custom name will overrule this.</dd>
	...
	/**
	 * Return all subparts of this entity. These parts are not saved in the chunk and do not tick, but are detected by getEntitiesInAABB and are put in the entity ID map. Vanilla makes the assumption that the entities in this array have consecutive entity ID's
	 * after their owner ID, so you must construct all parts in the constructor of the parent.
	 */
	 ...
	 *
	 * </dd>
	 * <dt>{@link ICommandSender#getDisplayName() ICommandSender.getDisplayName()} and {@link Entity#getDisplayName() Entity.getDisplayName()}</dt>
	...
	/**
	 * Checks if this thing has a custom name. This method has slightly different behavior depending on the interface (for <a href="https://github.com/ModCoderPack/MCPBot-Issues/issues/14">technical reasons</a> the same method is used for both IWorldNameable
	 * and Entity):
	 * 
	 * <dl>
	 ...
	 * <dd>If true, then {@link Entity#getCustomNameTag() Entity.getCustomNameTag()} will return a non-empty string, which will be used by {@link #getName()}.</dd>
	 * </dl>
	 */
