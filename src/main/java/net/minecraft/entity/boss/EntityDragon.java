...
public class EntityDragon extends EntityLiving implements IEntityMultiPart, IMob {
	...
	/**
	 * An array of bitmaps indicating, for each member of pathPoints, which other members should be considered "neighboring" for the purpose of path-finding. The set bits indicate the indexes into pathPoints that should be considered to be candidates for the
	 * next step in a path through the circles. Generated by initPathPoints.
	 */
		...
		if (this.getHealth() <= 0.0F) {
			...
			if (this.isAIDisabled()) {
				...
				for (int k = 0; k < 3; ++k) {
					...
					multipartentitypart.setLocationAndAngles(this.posX - (double) ((f3 * 1.5F + f6 * f24) * f16), this.posY + (adouble1[1] - adouble[1]) - (double) ((f24 + 1.5F) * f2) + 1.5D, this.posZ + (double) ((f18 * 1.5F + f22 * f24) * f16), 0.0F,
							0.0F);
				}
