...
import ru.itskekoff.client.render.CapeHolder;
...
public abstract class EntityPlayer extends EntityLivingBase implements CapeHolder {
	...
	/**
	 * Offset in the Y axis used for rendering. This field is not written to in vanilla (other than being set to 0 each tick by {@link net.minecraft.client.entity.EntityOtherPlayerMP#onUpdate()}).
	 */
	...
	private Simulation stickSimulation;
	...
	/**
	 * Called frequently so the entity can handler its state every tick as required. For example, zombies and skeletons use this to react to sunlight and start to burn.
	 */
	...
	/**
	 * Creates and drops the provided item. Depending on the dropAround, it will drop teh item around the player, instead of dropping the item from where the player is pointing at. Likewise, if traceItem is true, the dropped item entity will have the thrower
	 * set as the player.
	 * 
	...
	public SleepResult trySleep(BlockPos bedLocation) {
		...
		if (!this.world.isRemote) {
			...
			List<EntityMob> list = this.world.<EntityMob>getEntitiesWithinAABB(EntityMob.class,
					new AxisAlignedBB((double) bedLocation.getX() - 8.0D, (double) bedLocation.getY() - 5.0D, (double) bedLocation.getZ() - 8.0D, (double) bedLocation.getX() + 8.0D, (double) bedLocation.getY() + 5.0D, (double) bedLocation.getZ() + 8.0D),
					new SleepEnemyPredicate(this));
	 ...
	 * 
	 * <dl>
	 * <dd>Returns the name of this inventory. then this <em>should</em> be a direct string; otherwise it <em>should</em> be a valid translation string.</dd>
	 ...
	 * {@link net.minecraft.inventory.InventoryCraftResult InventoryCraftResult} (always returns <code>Result</code>) and the {@link EntityMinecart EntityMinecart} family (uses the entity definition). This is not an exaustive list.</dd>
	 * <dd>In general, this method should be safe to use on tile entities that implement IInventory.</dd>
	 * <dt>{@link net.minecraft.command.ICommandSender#getName() ICommandSender.getName()} and {@link Entity#getName() Entity.getName()}</dt>
