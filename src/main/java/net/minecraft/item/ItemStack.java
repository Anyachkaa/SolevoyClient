...
public final class ItemStack implements IItemStack {
	...
	/**
	 * Attempts to damage the ItemStack with par1 amount of damage, If the ItemStack has the Unbreaking enchantment there is a chance for each point of damage to be negated. Returns true if it takes more damage than getMaxDamage(). Returns false otherwise or
	 * if the ItemStack can't be damaged or if all points of damage are negated.
	 */
	...
	/**
	 * Called each tick as long the ItemStack in on player inventory. Used to progress the pickup animation and handler maps.
	 */
	...
	/**
	 * Returns whether this stack is always allowed to edit the world. Forces {@link EntityPlayer#canPlayerEdit EntityPlayer#canPlayerEdit} to return {@code true}.
	 * 
	...
	public Multimap<String, AttributeModifier> getAttributeModifiers(EntityEquipmentSlot equipmentSlot) {
		...
		if (this.hasTagCompound() && this.stackTagCompound.hasKey("AttributeModifiers", 9)) {
			...
			for (int i = 0; i < nbttaglist.tagCount(); ++i) {
				...
				if (attributemodifier != null && (!nbttagcompound.hasKey("Slot", 8) || nbttagcompound.getString("Slot").equals(equipmentSlot.getName())) && attributemodifier.getID().getLeastSignificantBits() != 0L
	...
	/**
	 * Returns whether this stack is explicitly allowed to be used on the given block via the {@code "CanPlaceOn"} NBT tag.
	 * 
