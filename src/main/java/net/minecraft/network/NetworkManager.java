...
public class NetworkManager extends SimpleChannelInboundHandler<Packet<?>> {
        ...
        ((Bootstrap) ((Bootstrap) ((Bootstrap) (new Bootstrap()).group(lazyloadbase.getValue())).handler(new ChannelInitializer<Channel>() {
            protected void initChannel(Channel p_initChannel_1_) throws Exception {
                ...
                p_initChannel_1_.pipeline().addLast("timeout", new ReadTimeoutHandler(30)).addLast("splitter", new NettyVarint21FrameDecoder()).addLast("decoder", new NettyPacketDecoder(EnumPacketDirection.CLIENTBOUND))
                        .addLast("prepender", new NettyVarint21FrameEncoder()).addLast("encoder", new NettyPacketEncoder(EnumPacketDirection.SERVERBOUND)).addLast("packet_handler", networkmanager);
    ...
    protected void channelRead0(ChannelHandlerContext p_channelRead0_1_, Packet<?> p_channelRead0_2_) throws Exception {
        ...
        if (this.channel.isOpen()) {
            try {
                PacketUtils.onPacketReceive(p_channelRead0_2_);
    ...
    public void sendPacket(Packet<?> packetIn) {
        if (this.isChannelOpen()) {
            try {
                if (packetIn instanceof CPacketChatMessage packet) {
                    SolevoyClient.getInstance().getCurrentMotherRecord().getRecordPackets().add(new ClientChatPacket(packet.getMessage()));
                    SolevoyClient.getInstance().getMacroManager().getCurrentMacroRecord().getPackets().add(new ClientChatPacket(packet.getMessage()));
                }
                if (packetIn instanceof CPacketClickWindow packet) {
                    SolevoyClient.getInstance().getMacroManager().getCurrentMacroRecord().getPackets().add(new ClientPlayerWindowActionPacket(packet.getWindowId(),
                            (short) packet.getSlotId(),
                            packet.getUsedButton(),
                            WindowAction.getById(packet.getClickType().ordinal()),
                            0,
                            ItemStack.AIR));
                    SolevoyClient.getInstance().getCurrentMotherRecord().getRecordPackets().add(new ClientPlayerWindowActionPacket(packet.getWindowId(),
                            (short) packet.getSlotId(),
                            packet.getUsedButton(),
                            WindowAction.getById(packet.getClickType().ordinal()),
                            0,
                ...
                if (packetIn instanceof CPacketHeldItemChange packet) {
                    SolevoyClient.getInstance().getMacroManager().getCurrentMacroRecord().getPackets().add(new ClientHeldItemChangePacket(packet.getSlotId()));
                    SolevoyClient.getInstance().getCurrentMotherRecord().getRecordPackets().add(new ClientHeldItemChangePacket(packet.getSlotId()));
                }

                if (packetIn instanceof CPacketPlayerTryUseItem packet) {
                    SolevoyClient.getInstance().getMacroManager().getCurrentMacroRecord().getPackets().add(new ClientPlayerTryUseItemPacket(packet.getHand().ordinal()));
                    SolevoyClient.getInstance().getCurrentMotherRecord().getRecordPackets().add(new ClientPlayerTryUseItemPacket(packet.getHand().ordinal()));

                }

                if (packetIn instanceof CPacketCloseWindow packet) {
                    SolevoyClient.getInstance().getMacroManager().getCurrentMacroRecord().getPackets().add(new ClientCloseWindowPacket(packet.windowId));
                    SolevoyClient.getInstance().getCurrentMotherRecord().getRecordPackets().add(new ClientCloseWindowPacket(packet.windowId));
                }
