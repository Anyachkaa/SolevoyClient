...
public class TileEntityStructure extends TileEntity {
	 ...
	 * @return true if the template was successfully saved.
	 * 
	 * @param writeToDisk If true, {@link TemplateManager#writeTemplate} will be called with the template, and its return value will dictate the return value of this method. If false, the template will be updated but not written to disk.
	 */
	 ...
	 * @return true if the template was successfully added to the world.
	 * 
	 * @param requireMatchingSize If true, and the size of the loaded template does not match the size in this structure block, the structure will not be loaded into the world and false will be returned. Regardless of the value of this parameter, the size in
	 *                            the structure block will be updated after calling this method.
	 */
		...
		if (this.mode == Mode.LOAD && !this.world.isRemote && !StringUtils.isNullOrEmpty(this.name)) {
			...
			if (template == null) {
				...
				if (requireMatchingSize && !flag) {
					...
					PlacementSettings placementsettings = (new PlacementSettings()).setMirror(this.mirror).setRotation(this.rotation).setIgnoreEntities(this.ignoreEntities).setChunk((ChunkPos) null).setReplacedBlock((Block) null)
