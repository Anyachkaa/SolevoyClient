...
public abstract class World implements IBlockAccess {
	...
	/**
	 * True while the World is ticking {@link #tickableTileEntities}, to prevent CME's if any of those ticks create more tile entities.
	 */
	private boolean processingLoadedTiles;
	private final WorldBorder worldBorder;

	/**
	 * is a temporary list of blocks and light values used when updating light levels. Holds up to 32x32x32 blocks (the maximum influence of a light source.) Every element is a packed bit value: 0000000000LLLLzzzzzzyyyyyyxxxxxx. The 4-bit L is a light level
	 * used when darkening blocks. 6-bit numbers x, y and z represent the block's offset from the original block, plus 32 (i.e. value of 31 would mean a -1 offset
	 */
	...
	/**
	 * Flag 1 will cause a block handler. Flag 2 will send the change to clients. Flag 4 will prevent the block from being re-rendered, if this is a client world. Flag 8 will force any re-renders to run on the main thread instead of the worker pool, if this is
	 * a client world and flag 4 is clear. Flag 16 will prevent observers from seeing this change. Flags can be OR-ed
	 */
	...
	/**
	 * Convenience method to handler the block on both the client and server
	 */
	...
	/**
	 * Marks a vertical column of blocks dirty, scheduling a render handler.
	 * 
	...
	/**
	 * Notifies all listening IWorldEventListeners of an handler within the given bounds.
	 */
	...
	/**
	 * Spawns particles that will always show regardless of the Video Settings -> Particles setting. Range restrictions still apply.
	 */
	...
	/**
	 * Gets all <strong>block</strong> collision boxes in the given area. Also gets a world border collision box. Does not get entity collision boxes.
	 * 
	...
	/**
	 * Called by CommandClone and AnvilChunkLoader to force a block handler.
	 */
	...
	/**
	 * Updates the entity in the world if the chunk the entity is in is currently loaded or its forced to handler.
	 */
	...
	/**
	 * Checks if the given block can be set at {@code pos}. {@code sidePlacedOn} is the side of the backing block that was clicked on to trigger this placement.
	 */
