...
public final class WorldEntitySpawner {
									...
									for (int i4 = 0; i4 < l3; ++i4) {
										...
										if (!worldServerIn.isAnyPlayerWithinRangeAt((double) f, (double) i3, (double) f1, 24.0D) && blockpos1.distanceSq((double) f, (double) i3, (double) f1) >= 576.0D) {
											...
											if (worldServerIn.canCreatureTypeSpawnHere(enumcreaturetype, biome$spawnlistentry, blockpos$mutableblockpos)
													&& canCreatureTypeSpawnAtLocation(EntitySpawnPlacementRegistry.getPlacementForEntity(biome$spawnlistentry.entityClass), worldServerIn, blockpos$mutableblockpos)) {
												...
												boolean flag2 = Reflector.ForgeEventFactory_canEntitySpawn.exists() ? ReflectorForge.canEntitySpawn(entityliving, worldServerIn, f, (float) i3, f1)
	...
	public static boolean canCreatureTypeSpawnBody(EntityLiving.SpawnPlacementType p_canCreatureTypeSpawnBody_0_, World p_canCreatureTypeSpawnBody_1_, BlockPos p_canCreatureTypeSpawnBody_2_) {
		...
		if (p_canCreatureTypeSpawnBody_0_ == EntityLiving.SpawnPlacementType.IN_WATER) {
			return iblockstate.getMaterial() == Material.WATER && p_canCreatureTypeSpawnBody_1_.getBlockState(p_canCreatureTypeSpawnBody_2_.down()).getMaterial() == Material.WATER
					&& !p_canCreatureTypeSpawnBody_1_.getBlockState(p_canCreatureTypeSpawnBody_2_.up()).isNormalCube();
		} else {
			...
			boolean flag = Reflector.ForgeBlock_canCreatureSpawn.exists() ? Reflector.callBoolean(iblockstate1.getBlock(), Reflector.ForgeBlock_canCreatureSpawn, iblockstate1, p_canCreatureTypeSpawnBody_1_, blockpos, p_canCreatureTypeSpawnBody_0_)
					: iblockstate1.isTopSolid();
