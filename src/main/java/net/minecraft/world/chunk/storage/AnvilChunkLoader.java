...
public class AnvilChunkLoader implements IChunkLoader, IThreadedFileIO, IAnvilChunkLoader {
	...
	public @Getter File chunkSaveLocation;
		...
		if (!compound.hasKey("Level", 10)) {
			...
			if (!nbttagcompound.hasKey("Sections", 9)) {
				...
				if (!chunk.isAtLocation(x, z)) {
					LOGGER.error("Chunk file at {},{} is in the wrong location; relocating. (Expected {}, {}, got {}, {})", Integer.valueOf(x), Integer.valueOf(z), Integer.valueOf(x), Integer.valueOf(z), Integer.valueOf(chunk.x),
	 ...
	 * 
	 * @return true if there are more IO actions to perform afterwards, or false if there are none (and this instance of IThreadedFileIO should be removed from the queued list)
	 */
	...
	/**
	 * Writes the Chunk passed as an argument to the NBTTagCompound also passed, using the World argument to retrieve the Chunk's last handler time.
	 */
